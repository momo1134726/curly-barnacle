...
export class CollectionScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, height * 0.1, 'コレクション', { fontSize: '32px', color: '#e94560', fontStyle: 'bold' }).setOrigin(0.5);
    this.add.text(width / 2, height * 0.5, '（一覧実装予定）', { fontSize: '20px', color: '#888888' }).setOrigin(0.5);
    this._backButton();
  }
